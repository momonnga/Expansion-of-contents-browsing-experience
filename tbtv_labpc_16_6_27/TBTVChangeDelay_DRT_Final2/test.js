...
$.ajax({
  ...
  var codeInit="function initialize(){can=document.getElementById('can');ctx=can.getContext('2d');try{player = document.getElementById('external_nicoplayer');}catch(e){}try{playerY = document.getElementById('movie_player');}catch(e){}effectArr=effect.split(',');vposStartTArr=vposStartT.split(',');vposEndTArr=vposEndT.split(',');effectTArr=effectT.split(',');startYArr=startY.split(',');endYArr=endY.split(',');ww=window.innerWidth;wh=window.innerHeight;sw=window.screen.width;sh=window.screen.height;if(effectTArr.length>0&&effectTArr[0]!=''){type=effectTArr[0];can.width=ww;can.height=wh;}else{}for(var i=0;i < 60;i++){array[i] = new Array();for(var j=0;j < 30;j++){array[i][j] = 150;}}for(var i=0;i<5;i++){arrx[i]=-1;arry[i]=-1;}dateL=new Date();secondsL=dateL.getTime();sp1=new StarPre();sp2=new StarPre();sp3=new StarPre();sp4=new StarPre();sp5=new StarPre();sp6=new StarPre();sp7=new StarPre();sp8=new StarPre();sp9=new StarPre();sp10=new StarPre();sp11=new StarPre();sp12=new StarPre();sp13=new StarPre();sp14=new StarPre();sp15=new StarPre();sp16=new StarPre();sp17=new StarPre();sp18=new StarPre();sp19=new StarPre();sp20=new StarPre();ready=1;can.width=0;can.height=0;}";
  var codeInitR="function initializeR(){canR=document.getElementById('canR');ctxR=canR.getContext('2d');try{player = document.getElementById('external_nicoplayer');}catch(e){}try{playerY = document.getElementById('movie_player');}catch(e){}effectArr=effect.split(',');vposStartTArr=vposStartT.split(',');vposEndTArr=vposEndT.split(',');effectTArr=effectT.split(',');startYArr=startY.split(',');endYArr=endY.split(',');ww=window.innerWidth;wh=window.innerHeight;sw=window.screen.width;sh=window.screen.height;if(effectTArr.length>0&&effectTArr[0]!=''){type=effectTArr[0];canR.width=ww;canR.height=wh;}else{}for(var i=0;i < 60;i++){array[i] = new Array();for(var j=0;j < 30;j++){array[i][j] = 150;}}for(var i=0;i<5;i++){arrx[i]=-1;arry[i]=-1;}dateL=new Date();secondsL=dateL.getTime();spR1=new StarPre();spR2=new StarPre();spR3=new StarPre();spR4=new StarPre();spR5=new StarPre();spR6=new StarPre();spR7=new StarPre();spR8=new StarPre();spR9=new StarPre();spR10=new StarPre();spR11=new StarPre();spR12=new StarPre();spR13=new StarPre();spR14=new StarPre();spR15=new StarPre();spR16=new StarPre();spR17=new StarPre();spR18=new StarPre();spR19=new StarPre();spR20=new StarPre();canR.width=0;canR.height=0;}";
  ...
  var codeM = function(e) {
    ...
    if(ready==1){
      arJSON[countMLC%arJSON.length]=JSON.parse(e.data);
    }
    /*/
    if(countM1==0){
    ...
    /*/
    //console.log(countMLC%arJSON.length);
    //console.log(arJSON[countMLC%arJSON.length]);
    countMLC++;
    if(countMLC%DLevel==0){
      countMLC=0;
      if(countM1==0){
        countM1=1;
      }
  ...
  if(countM1==1){
    ...
    if(ready==1){
      ...
      //console.log((countMLC+arJSOND)%DLevel-countMLC);

       try{
        resultCMLC=(countMLC+1)%DLevel;
        if(arJSON[resultCMLC].eX<0){
          cx=arJSON[resultCMLC].eX+1922;
              //cx=ww-Math.abs(arJSON.eX);
        }else{
            cx=arJSON[resultCMLC].eX;
        }
        cy=arJSON[resultCMLC].eY-61;
      }catch(e){
        try{
          //console.log("1"+e);
          //console.log(e);
          resultCMLC=(countMLC+2)%DLevel;
          if(arJSON[resultCMLC].eX<0){
            cx=arJSON[resultCMLC].eX+1922;
                //cx=ww-Math.abs(arJSON.eX);
          }else{
      ...
      if(type=="NULL"){
        ...
        ctx.arc(cx, cy, 400-i*10, 0, Math.PI*2, true);
        ctx.fill();
        }
        /* 合成演算子をセット */
      ...
      if(recordFlag==1){
        ...
        if(id!="sm9"){
            recordPos[recordPos.length]=type+","+cxLog+","+cyLog+","+document.body.scrollLeft+","+document.body.scrollTop+","+Year+"-"+Month+"-"+Day+"-"+H+"-"+M+"-"+S+"-"+MS+","+player.ext_getStatus()+","+player.ext_getPlayheadTime()+","+player.ext_getVideoSize()+","+arJSON[resultCMLC].TimeMS+","+DLevel+"\r\n";
        }else if(exp=="experiment"){
          recordPos[recordPos.length]=type+","+cxLog+","+cyLog+","+document.body.scrollLeft+","+document.body.scrollTop+","+Year+"-"+Month+"-"+Day+"-"+H+"-"+M+"-"+S+"-"+MS+","+getCurrentPS()+","+getCurrentVT()+","+arJSON[resultCMLC].TimeMS+","+DLevel+"\r\n";
        }else{
          recordPos[recordPos.length]=type+","+cxLog+","+cyLog+","+document.body.scrollLeft+","+document.body.scrollTop+","+Year+"-"+Month+"-"+Day+"-"+H+"-"+M+"-"+S+"-"+MS+","+arJSON.TimeMS+"\r\n";    
        }
      }
    }
  }
  //setTimeout(listenTobii,0);
}; 
var codeL = function(){
...
var codeK = function(evt){
  ...
  if(evt.which == 69){// e
    experimentFlag++;
    experimentFlag=experimentFlag%2;
    if(experimentFlag==1){
      canR.width=ww;
      canR.height=wh;
      ctxR.fillStyle = "rgba(169,169,169,1)";
      ctxR.fillRect(0,0,ww,wh);
    }else{
      canR.width=0;
      canR.height=0;
