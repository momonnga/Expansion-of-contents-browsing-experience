...
if(bar!=null){
    id=bar[0];
        ...
        if(id!="sm9"){
          var targetURL2="http://localhost/effect/AnalyzeContentMPN.php?sm="+id;
$.ajax({
    url: targetURL2,
    dataType: 'json',
    async: false,
 ...
 //alert("MySQL動いてる？");
           ...
           var targetURL="http://localhost/effect/AnalyzeEffectM.php?id="+id;
...
$.ajax({
    ...
    //location.href="http://localhost/effect/AnalyzeContentM.php?sm="+id;
        ...
        var targetURL2="http://localhost/effect/AnalyzeContentTPN.php?url="+nowURL;
...
var targetURL="http://localhost/effect/AnalyzeEffectT.php?url="+nowURL;
$.ajax({
    url: targetURL,
    dataType: 'json',
    async: false,
  ...
  //console.log(Math.floor((secondsR-secondsL)/100));
  //console.log(Math.floor(vposSend/100));
  //if(Math.floor((secondsR-secondsL)/100)==Math.floor(vposSend/100)){
  if(player!=null){
   var videoTime=player.ext_getPlayheadTime();
   /*/
   if(sw<1920||sw<2100){//notePC
    cy=cy-250;
    cy=(cy*(wh+125))/sh;
   }else if(sw>2100){
    cy=cy-160;
  ...
  if(sh==768){//ノートPCVerNEC
   ...
   cx=(cx*ww)/sw;
   //console.log(Math.floor(videoTime));
   //console.log(Math.floor(arrayOfStrings[1]/100));
...
if(recordFlag==1){
  ...
  if(id!="sm9"){
      recordPos[recordPos.length]=type+","+cxLog+","+cyLog+","+document.body.scrollLeft+","+document.body.scrollTop+","+Year+"-"+Month+"-"+Day+"-"+H+"-"+M+"-"+S+"-"+MS+","+player.ext_getStatus()+","+player.ext_getPlayheadTime()+","+player.ext_getVideoSize()+"\r\n";
  }else{
    recordPos[recordPos.length]=type+","+cxLog+","+cyLog+","+document.body.scrollLeft+","+document.body.scrollTop+","+Year+"-"+Month+"-"+Day+"-"+H+"-"+M+"-"+S+"-"+MS+"\r\n";
  }
}
  ...
  if(type=="NULL"){
    ...
    //ctx.lineWidth=i*40;
    ctx.arc(cx, cy, 250-i*10, 0, Math.PI*2, true);
    ctx.fill();
    }
    ...
    ctx.lineWidth=2;
        for(var i=0;i<ww;i+=50){
          ...
          ctx.lineTo(i+((cx-i)*0.9)*rnd, cy*0.9*rnd);
          ctx.stroke();
          ...
          ctx.moveTo(i, wh);
          //ctx.lineTo(cx*0.8, cy*0.8);
          ctx.lineTo(i+((cx-i)*0.9)*rnd2, wh-((wh-cy)*0.9)*rnd2);
          ctx.stroke();
        }
        ...
        for(var i=0;i<wh;i+=50){
          ...
          ctx.moveTo(0, i);
          //ctx.lineTo(cx*0.8, cy*0.8);
          ctx.lineTo(cx*0.9*rnd, i+((cy-i)*0.9)*rnd);
          ctx.stroke();
          ...
          ctx.moveTo(ww, i);
          //ctx.lineTo(cx*0.8, cy*0.8);
          ctx.lineTo(ww-((ww-cx)*0.9)*rnd2, i+((cy-i)*0.9)*rnd2);
          ctx.stroke();
        }
  }else if(type=="Mosaic"){
  ...
  if(typeR=="NULL"){
    ...
    leng=0.7;
        for(var i=0;i<ww;i+=50){
          ...
          ctxR.moveTo(i, 0);
          //ctx.lineTo(cx*0.8, cy*0.8);
          ctxR.lineTo(i+((cxR-i)*leng)*rnd, cyR*leng*rnd);
          ctxR.stroke();
          ...
          ctxR.moveTo(i, wh);
          //ctx.lineTo(cx*0.8, cy*0.8);
          ctxR.lineTo(i+((cxR-i)*leng)*rnd2, wh-((wh-cyR)*leng)*rnd2);
          ctxR.stroke();
        }
        ...
        for(var i=0;i<wh;i+=50){
          ...
          ctxR.moveTo(0, i);
          //ctx.lineTo(cx*0.8, cy*0.8);
          ctxR.lineTo(cxR*leng*rnd, i+((cyR-i)*leng)*rnd);
          ctxR.stroke();
          ...
          ctxR.moveTo(ww, i);
          //ctx.lineTo(cx*0.8, cy*0.8);
          ctxR.lineTo(ww-((ww-cxR)*leng)*rnd2, i+((cyR-i)*leng)*rnd2);
          ctxR.stroke();
        }
